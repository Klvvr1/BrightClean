                ...
                var agent = await _context.LaundryAgents
                    ...
                    .Select(a => new
                    {
                        ...
                        services = a.SubscribedServices
                            .Where(s => s.ServiceCatalogItem != null)
                            .Select(s => new
                            {
                                s.ServiceID,
            ...
            var agent = await _context.LaundryAgents
                ...
                .Select(a => new
                {
                    ...
                    serviceIds = a.SubscribedServices
                        .Where(s => s.IsActive && (!s.PendingActivation || s.RequestedAction != AgentServiceRequestedAction.Activate) && s.ServiceCatalogItem.IsAvailable && !s.ServiceCatalogItem.IsDeleted)
                        .Select(s => s.ServiceID)
                        .ToList(),
                    serviceCount = a.SubscribedServices
                        .Count(s => s.IsActive && (!s.PendingActivation || s.RequestedAction != AgentServiceRequestedAction.Activate) && s.ServiceCatalogItem.IsAvailable && !s.ServiceCatalogItem.IsDeleted),
